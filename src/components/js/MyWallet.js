...
root.methods.testUID_01 = function () {
  ...
  if (this.$globalFunc.testSpecial(this.testUID)) {
    this.testUIDMsg_0 = this.$t('nameMsg_1')
    return false
  }
  this.testUIDMsg_0 = ''
  return true
...
root.methods.re_commitEmailVerification = function (data) {
  ...
  if (data.errorCode) {
    if (data.errorCode === 1) {
      this.emailVerificationSending = false
      this.popText = this.$t('iKnowthe2')
      this.popType = 0
      this.popOpen = true
    }
    if (data.errorCode === 2) {
      this.emailVerificationSending = false
      this.popText = this.$t('iKnowthe3')
      this.popType = 0
      this.popOpen = true
    }
    if (data.errorCode === 3) {
      this.emailVerificationSending = false
      this.popText = this.$t('iKnowthe4')
      this.popType = 0
      this.popOpen = true
    }
    if (data.errorCode === 4) {
      this.emailVerificationSending = false
      this.popText = this.$t('iKnowthe5')
      this.popType = 0
      this.popOpen = true
    }
    // this.popWindowStep = 2
...
root.methods.getEmailVerification = function () {
  ...
  this.$http.send('POST_VERIFICATION_CODE', {
    ...
    params: {
      type: 'email',
      purpose: 'transfer',
      transferTime: this.formatDateUitl(this.serverTime),
