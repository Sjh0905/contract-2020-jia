...
root.data = function () {
	return {
		...
		langs: {
			...
			two: this.$store.state.lang == 'CH' ? '活动内容 : 活动期间凡参与ELF交易的用户，按ELF交易量（买入量+卖出量）进行排名发放BDB奖励' : 'Details: To celebrate ælf listing, EUNEX offers extra bonus for valuable ELF market members. Top 50 members with high volume ELF transactions (sum of both ELF Buy and Sell amount) will get:',
			...
			recharge_four_two: this.$store.state.lang == 'CH' ? '如发现恶意交易等不当行为，EUNEX有权取消活动资格及奖励' : 'Please note that any behavior of unfair competition will be disqualified immediately',
...
root.watch.lang = function (newValue, oldValue) {
	...
	this.langs = {
		...
		two: newValue == 'CH' ? '活动内容 : 活动期间凡参与ELF交易的用户，按ELF交易量（买入量+卖出量）进行排名发放BDB奖励' : 'Details: To celebrate ælf listing, EUNEX offers extra bonus for valuable ELF market members. Top 50 members with high volume ELF transactions (sum of both ELF Buy and Sell amount) will get:',
		...
		five_two: newValue == 'CH' ? '如发现恶意交易等不当行为，EUNEX有权取消活动资格及奖励' : 'Any behavior of unfair competition will be disqualified immediately',
		five_three: newValue == 'CH' ? '本活动最终解释权归EUNEX所有' : 'EUNEX reserves the right of further adjustment and final interpretation of this announcement',
		...
		recharge_four_two: newValue == 'CH' ? '如发现恶意交易等不当行为，EUNEX有权取消活动资格及奖励' : 'Please note that any behavior of unfair competition will be disqualified immediately',
		recharge_four_three: newValue == 'CH' ? '本活动最终解释权归EUNEX所有' : 'EUNEX reserves the right of further adjustment and final interpretation of this announcement',
		List: this.$store.state.lang == 'CH' ? '序号' : 'List',
	}
}
