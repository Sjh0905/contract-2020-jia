...
Datafeeds.UDFCompatibleDatafeed = function (datafeedURL, configurationData, updateFrequency) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function (callback) {
	...
	this._send(urlHead+'/v1/common/timestamp').done(function (response) {

		console.log('~~~~~~22', response)


		var time = parseInt(response.timestamp / 1000);

		// console.log('~~~~~~',time,new Date(response.timestamp))

		// var time = parseInt(response.result / 1000);
		if (!isNaN(time)) {
			callback(time);
		}
	})
		.fail(function () {
			console.warn('failed get timestamp.');
		});
};
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	var data = {
		...
		'exchange-traded': 'BIT',
		'exchange-listed': '',
		timezone: that._configuration.timezone,
		minmov: 1, //最小波动
		minmov2: 0,
		pointvalue: 1,//价格精度
		session: '24x7',
		...
		has_no_volume: false,
		type: 'stock',
		supported_resolutions: that._configuration.supported_resolutions,
		pricescale: 100,
		ticker: symbolName
	}
	...
	// this._send(urlHead+'/v1/common/symbols/' + symbolName)
	// 	.done(function (response) {
	//
	//
	// 		if (response) {
	...
	// 			var data = {
	// 				name: symbolName,
	// 				description: r.base.name + '/' + r.quote.name,
	// 				'exchange-traded': 'BIT',
	// 				'exchange-listed': '',
	// 				timezone: that._configuration.timezone,
	// 				minmov: 1, //最小波动
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	// console.log('+~~~~~getBars======')
	// console.log('+~~~~~rangeStartDate',rangeStartDate,(new Date(rangeStartDate*1000)).toLocaleString())
	// console.log('+~~~~~rangeEndDate',rangeEndDate,(new Date(rangeEndDate*1000)).toLocaleString())
	...
	this._send(urlHead+'/v1/market/bars/' + symbolInfo.ticker + '/' + resolution_mapping[resolution], {
		start: rangeStartDate * 1000,
		...
		.done(function (response) {
			...
			if (response) {
				...
				onDataCallback(bars, {noData: bars.length === 0, nextTime: undefined});
				data.length === 0 && (able_socket = true);

			} else {
				if (!!onErrorCallback) {
					onErrorCallback(response.result.message);
				}
			}
		})
		.fail(function (arg) {
	...
	// socket 请求数据 @lf
	able_socket && Vue.$socket.on({
		key: 'topic_bar',
		bind: Vue,
		callBack: function (message) {
			console.log('there is bars:', bars);
			return;
			...
			Datafeeds.DataPulseUpdater(bars, 1000);
			return;
			// onDataCallback(bars, {noData: true, nextTime: undefined});
		}
	})
